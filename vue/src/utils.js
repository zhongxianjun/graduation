...
function getPrivateArr(privareArr, otherArr){
  mesArr = [];
  otherMesArr = [];
 
  allMesArr.map((item, index)=>{
    if(item.private == 0){
      otherArr.push(item);
    }else{
      privareArr.push(item);
    }
  })

  privareArr = unique(privareArr);
  otherArr = unique(otherArr);

  console.log(otherArr);
};


getPrivateArr(mesArr, otherMesArr);
